...
1. What does it mean for a tree to be undocumented? What options are there? 
``` {r}
summary(sfo_trees_df)
...
sfo_trees_df %>% 
  ...
  ggplot(aes(x=fct_reorder(legal_status,tree_count), y=tree_count))+
  geom_bar(stat="identity")+
  labs(title="Legal Status of SFO's Trees", caption="")+
  xlab("Legal Status")+
  ylab("Count")+
  theme_bw()+
...
tree_plot = ggplot(slice_df, aes(x=legal_status, y=tree_count))+
  geom_bar(stat="identity")+
  labs(title="Legal Status of SFO's Trees", caption="")+
  xlab("Legal Status")+
  ylab("Count")+
...
Filters the sfo_trees_df database for those where the Permitted Site is "Permitted Site" and the caretaker is either MTA or DPW
```{r}
...
Allows the result to be true for any of the options within the created vector
```{r}
...
- filter for oak & pine
- select for legal status, location, planting date
- summarize counts by legal status
...
ggplot(oak_pine_df, aes(x=longitude,y=latitude,color=type))+
  geom_point()+
  labs(title="SFO Trees by Type", color="Tree Type")+
  theme_minimal()
```
...
We have a list of native trees in the data folder.
...
t.test(plant_year~spp_sci,data=oak_pine_year_df) # p=2.69e-5
```
...
st_transform(sfo_map, 4326) # sets the reference system
```
...
ggplot()+
  ...
  geom_sf(data=oak_pine_sf,aes(color=spp_sci),size=0.25)+
  theme_void()+
